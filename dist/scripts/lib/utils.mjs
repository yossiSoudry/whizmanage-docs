...
export function helperSearch(query, node, prefix, currentLevel, maxLevel) {
    const res = [];
    let parentHas = false;
...
function calculateProximityScore(query, content) {
...
function cleanMdxContent(content) {
    ...
    sanitizedContent = sanitizedContent.replace(/&(#(?:\d+)|(?:[a-z]+));/gi, (_, entity) => {
...
export function advanceSearch(query) {
    ...
    const results = chunks.flatMap((chunk) => chunk
        ...
        return (doc.relevance > 0 &&
            queryWords.some((word) => doc.title.toLowerCase().includes(word) ||
                (doc.description &&
...
export function highlight(snippet, searchTerms) {
